  ...
  while (argindex < argc && StrStartWith(argv[argindex], "-")) {
    if (strcmp(argv[argindex], "-threads") == 0) {
      ...
      argindex++;
        ...
        props[0].SetProperty("LDCStartLevel", argv[argindex]);
        argindex++;
    }  else if (strcmp(argv[argindex], "-LDCEndLevel") == 0) {
        argindex++;
        if (argindex >= argc) {
            UsageMessage(argv[0]);
            exit(0);
        }
        props[0].SetProperty("LDCEndLevel", argv[argindex]);
        argindex++;
    }else if (strcmp(argv[argindex], "-LDCMergeSizeRatio") == 0) {
        argindex++;
        if (argindex >= argc) {
            UsageMessage(argv[0]);
            exit(0);
        }
        props[0].SetProperty("LDCMergeSizeRatio", argv[argindex]);
        argindex++;
    }else if (strcmp(argv[argindex], "-UseAdaptiveLDC") == 0) {
        argindex++;
        if (argindex >= argc) {
            UsageMessage(argv[0]);
            exit(0);
        }
        props[0].SetProperty("UseAdaptiveLDC", argv[argindex]);
        argindex++;
    }else if (strcmp(argv[argindex], "-ThresholdBufferNum") == 0) {
        argindex++;
        if (argindex >= argc) {
            UsageMessage(argv[0]);
            exit(0);
